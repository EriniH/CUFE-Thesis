...
The developed framework provides a unified command-driven environment that enables users to interact directly with simulation results through a programmable interface. It integrates waveform management, numerical measurement extraction, mathematical expression evaluation, and automated visualization within a single
workflow.

The framework is based on an in-memory Waveform Database that stores and organizes
simulation results after parsing standard SPICE raw files. This database provides a
common data representation that allows different processing modules, including the
Measurement Engine, Calculation Engine, and Visualization Engine, to operate
efficiently on the same waveform data.

For visualization, the framework utilizes Gnuplot as an external rendering backend.
The Plot Engine is responsible for preparing waveform data and generating the
required plotting instructions, while Gnuplot performs the final graphical
rendering. This separation provides a flexible visualization architecture while
keeping the analysis modules independent from the plotting implementation.

This chapter presents the design and implementation of the proposed post-processing
framework. It describes the system architecture, waveform data model, command
interface, processing algorithms, visualization pipeline, and validation scenarios
used to evaluate the accuracy, robustness, and reliability of the developed tool.
\newpage
...
The error propagation mechanism relies on a hybrid approach of C++ exceptions for fatal I/O operations and state-based return structs for domain logic. This separation follows the principle that exceptions are reserved for unexpected system failures, while expected computational failures are represented explicitly through return values. This ensures predictable control flow and avoids the performance overhead of stack unwinding during heavy simulation loops, as illustrated in Figure~\ref{fig:error_handling_strategy}.
...
The strategy dictates the following workflow:
\begin{enumerate}
    \item \textbf{Input Validation:} The \texttt{Interpreter} performs primary syntax and argument validation. If a command is malformed, it immediately halts execution and prints an \texttt{[ERROR]} alongside helpful hints (e.g., fuzzy matching suggestions).
    \item \textbf{Exception Handling (I/O):} The File I/O layer uses standard C++ exceptions (\texttt{std::runtime\_error}) because missing or corrupted data files are fundamental failures. The interpreter catches these exceptions gracefully.
    \item \textbf{State-based Error Returns (Engines):} To avoid the performance overhead and control-flow ambiguity of exceptions in heavy computational tasks, the \texttt{MeasEngine} and \texttt{CalcEngine} return wrapper structs (e.g., \texttt{MeasResult}, \texttt{CalcResult}). These structs contain a boolean \texttt{valid} flag and an \texttt{error} string. 
    \item \textbf{Interception and Formatting:} The \texttt{Interpreter} inspects the returned struct. If \texttt{valid} is false, it intercepts the raw error string, formats it with the standard \texttt{[ERROR]} prefix, and prints it to \texttt{std::cerr}.
\end{enumerate}

The following tables provide selected examples of how this strategy manifests across the different modules.

\subsubsection{Selected Error Examples}
...
A dedicated evaluation was performed on periodic signals (sinusoidal and square waves) to evaluate the engine's boundary-condition logic.
Commands like \texttt{freq}, \texttt{period}, and periodic \texttt{rms} inherently require a minimum of two full positive zero-crossings to yield valid results.
The evaluation verified that the engine successfully measures periodic parameters when sufficient data is available. Crucially, when subjected to a simulation window that covers exactly one period or less, the framework accurately detected the boundary violation and aborted the frequency calculation, preventing the issuance of erroneous infinite or zero frequencies. Furthermore, the engine successfully computed multi-signal metrics, such as the phase \texttt{delay} between two independent square waves.
